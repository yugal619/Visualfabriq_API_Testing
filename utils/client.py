...
class Client():
    ...
    def verify_valid_data(self, kwargs, fail_on_error=True):
        ...
        assert response.status_code == 200, f"[FAILURE] Status code is not correct. \n Actual - {response.status_code}" \
                                            f"\nExpected = {200}"
        value = kwargs.get("expected_output")
        assert value in response.json()['message'], f'[FAILURE] Actual output - {response.json()["message"]} \n' \
                                                    f'Expected output - {value}'

    def verify_invalid_data(self, kwargs, fail_on_error=True):
        uri = '/Prod/next-birthday'
        response = RequestsUtils.get(host=self.base_url, path=uri, headers=self.header, params=kwargs,
                                      fail_on_error=fail_on_error)
